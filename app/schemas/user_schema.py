...
class UserCreate(BaseModel):
    ...
    user_id: Optional[str] = Field(None, min_length=1, max_length=50, description="ID del usuario en el reloj. Si no se manda, se usa el UID.")
    ...
    def default_user_id_from_uid(self):
        if not self.user_id:
            self.user_id = str(self.uid)
        return self
